  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (message.trim() && !isLoading) {
      ...
      try {
        ...
        const imageKeywords = ['create', 'generate', 'make', 'design', 'draw', 'build', 'produce'];
        ...
        if (isImageRequest) {
          try {
            ...
            const imageUrl = await imageGenerationService.generateImage(userMessage);
            
            // Add the generated image to the library (simulate adding to images array)
            const newImage = {
              id: Date.now(),
              src: imageUrl,
              title: `Generated: ${userMessage.substring(0, 30)}${userMessage.length > 30 ? '...' : ''}`,
              category: 'generated',
              date: new Date().toISOString().split('T')[0],
              favorite: false,
              tags: ['generated', 'ai', 'dalle']
            };
            
            // In a real app, you would update the images state here
            console.log('Generated image:', newImage);
            
            sendMessage(sessionId, { 
              text: `✅ **Image Generated Successfully!**\n\nI've created an image based on your prompt: "${userMessage}"\n\nThe image has been generated using DALL-E and would normally be added to your library. You can view it at: ${imageUrl}\n\n*Note: In a full implementation, this image would be automatically saved to your library.*`, 
              sender: 'bot' 
            });
          } catch (error) {
            console.error('Image generation error:', error);
            sendMessage(sessionId, { 
              text: `❌ **Image Generation Failed**\n\nI encountered an error while trying to generate your image: ${error instanceof Error ? error.message : 'Unknown error'}\n\nPlease try again with a different prompt or check your API configuration.`, 
              sender: 'bot' 
            });
          }
        } else {
          // Handle as regular text conversation
          const aiResponse = await geminiService.generateLibraryResponse(userMessage, messages);
