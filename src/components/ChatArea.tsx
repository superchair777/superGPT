...
const ChatArea: React.FC<ChatAreaProps> = ({ sidebarOpen }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (message.trim() && !isLoading) {
      const newMessage: Message = { text: message, sender: 'user', timestamp: new Date().toISOString() };
      ...
      try {
        ...
        sendMessage(sessionId, { ...botMessage, timestamp: new Date().toISOString() });
        ...
        sendMessage(sessionId, { ...errorMessage, timestamp: new Date().toISOString() });
      } finally {
        setIsLoading(false);
      }
    }
